    ...
    if tag == 'instrument':
        ...
        for contour in contours:
            ...
            if tag == 'polyp' and area < 1000:
                ...
                continue
            elif (tag == 'ulcerative_colitis' or tag == 'oesophagitis') and area < 1000:
                continue
                ...
                try:
                    with Image.open(mask_path) as mask:
                        ...
                        if self.task == '<REFERRING_EXPRESSION_SEGMENTATION>':
                            answer = mask_to_florence_string(mask, tag=self.tag if self.tag in ['polyp', 'instrument', 'instrument_v2', 'ulcerative_colitis','oesophagitis'] else None)
                        elif self.task == '<OD>':
